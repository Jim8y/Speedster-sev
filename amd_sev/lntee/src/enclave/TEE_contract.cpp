#include <enclave/TEE_contract.h>

TEE_Contract::TEE_Contract() {};

void TEE_Contract::loadContract(nlohmann::json contract_definition, eevm::Address *owner) {
    DEBUG("");
    // Create environment
...
int TEE_Contract::rand_range(int exclusive_upper_bound) {
...
uint256_t TEE_Contract::get_random_uint256(size_t bytes) {
...
std::string TEE_Contract::short_name(const eevm::Address &address) {
...
std::vector<uint8_t> TEE_Contract::run_and_check_result(
...
// execution
void TEE_Contract::append_argument(
 ...
 * @return
 */
eevm::Address TEE_Contract::deploy_lntee_contract() {
...
uint256_t TEE_Contract::get_total_supply() {
...
// contract_address
uint256_t TEE_Contract::get_balance(const eevm::Address &target_address) {
...
void TEE_Contract::print_lntee_state(const std::string &heading) {
