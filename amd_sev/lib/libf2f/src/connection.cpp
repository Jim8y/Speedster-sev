...
namespace libf2f {
    ...
    Connection::Connection(std::shared_ptr<boost::asio::ip::tcp::socket> p_socket,
                           Router *r) : p_m_socket(p_socket) {
        m_ready = false;
        m_sending = false;
...
/// async_read() -> handle_read_headers() -> handle_read_data()
    void Connection::async_read() {
        ...
        boost::asio::async_read(*p_m_socket,
...
/// called when we've read the header off the wire
    void Connection::handle_read_header(const boost::system::error_code &e, message_ptr msgp) {
...
/// called when we've read header and payload off the wire
    void Connection::handle_read_data(const boost::system::error_code &e, message_ptr msgp) {
        if (m_shuttingdown)
            return;
        if (e) {
    ...
    Connection::str() const {
        ...
        os << "[Connection: '"
           << m_name << "' ";
        if (p_m_socket->is_open()) {
            try {
                os << p_m_socket->remote_endpoint().address().to_string()
                   << ":"
                   << p_m_socket->remote_endpoint().port();
            }
            catch (...) {
            }
        }
...
/// completes, to send the next msg in the queue. Bails when none left.
    ...
    void Connection::push_message_received_cb(boost::function<void(message_ptr, connection_ptr)> cb) {
        m_message_received_cbs.push_back(cb);
    }

    void Connection::pop_message_received_cb() {
