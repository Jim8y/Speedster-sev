...
int ecall_verify_backup_stored(const char *blob, int blob_len, char* encrypted_data_out, int* encrypted_data_out_len, char* p_gcm_mac, sgx_ra_context_t context, char *next_hop_channel_id, int* routing_complete, char *next_channel_id_to_send_on, int* send_action) {
    struct BackupStoredAckMsg msg;
    if (!check_and_decrypt_message(blob, blob_len, context, sizeof(struct BackupStoredAckMsg), (unsigned char*) &msg)) {
