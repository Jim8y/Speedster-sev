...
namespace eevm {
    ...
    private:
        ...
        void  (*debug_ptr)(const char* );

        using ET = Exception::Type;

    public:
        _Processor(GlobalState &gs, Transaction &tx, Trace *,void  (*fun_ptr)(const char* )) :
                gs(gs),
                tx(tx),
                tr(tr),
                debug_ptr(fun_ptr){}
...
//        tr->add(ctxt->get_pc(), op, get_call_depth(), ctxt->s);
            ...
            switch (op) {
                ...
                default:
                    stringstream err;
                    (*this->debug_ptr)("=== processor 625\n");
//                    (*this->debug_ptr)("=== processor 639");
//                    err << fmt::format(
...
//        (*this->debug_ptr)("=== processor 642");
    ...
    ExecResult Processor::run(
            ...
            const uint256_t &call_value,
            Trace *tr) {
        return _Processor(gs, tx, tr, debug_ptr).run(caller, callee, input, call_value);
    }
} // namespace eevm
