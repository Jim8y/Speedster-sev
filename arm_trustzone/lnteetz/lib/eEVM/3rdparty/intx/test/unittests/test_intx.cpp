...
for (
size_t i = 0;
i < sizeof(data_set) / sizeof(data_set[0]); ++i)
{
// if (i != 5) continue;
...
EXPECT_EQ(res
.quot, expected_q) << "data index: " <<
i;
EXPECT_EQ(res
.rem, expected_r) << "data index: " <<
i;
...
{
EXPECT_EQ(exp(3_u256, 0_u256),
1);
EXPECT_EQ(exp(3_u256, 1_u256),
3);
EXPECT_EQ(exp(3_u256, 2_u256),
9);
...
EXPECT_EQ(w,
15);
EXPECT_EQ(count_significant_words<uint8_t>(0_u256),
0);
}
...
for (
unsigned shift = 0;
shift < sizeof(TypeParam) * 8; ++shift)
{
auto x = TypeParam{1};
auto y = x << shift;
auto z = y >> shift;
...
for (
unsigned pos = 0;
pos < sizeof(TypeParam) * 8; ++pos)
EXPECT_NE((TypeParam{1}
<< pos) & ones, 0);
}
...
EXPECT_EQ(data[sizeof(x) - 1],
1);
EXPECT_EQ(data[sizeof(x) - 2],
2);
EXPECT_EQ(data[sizeof(x) - 3],
0);
EXPECT_EQ(data[0],
0);
}
...
const auto x = be::load<TypeParam>(s);
EXPECT_EQ(x, TypeParam{0x88}
<< (TypeParam::num_bits - 8));
}
...
EXPECT_EQ(to_string(x, 10),
"1024");
EXPECT_EQ(to_string(x, 16),
"400");
EXPECT_EQ(to_string(x, 36),
"sg");
EXPECT_EQ(to_string(x, 2),
"10000000000");
EXPECT_EQ(to_string(x, 8),
"2000");
}
