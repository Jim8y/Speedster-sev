...
namespace libf2f {
    ...
    public:
        Connection(
                std::shared_ptr<boost::asio::ip::tcp::socket> p_socket, Router *r);
        ...
        void do_async_write(
                const boost::system::error_code &e, message_ptr finished_msg);
        ...
        void handle_read_header(
                const boost::system::error_code &e, message_ptr msgp);
        ...
        size_t writeq_size() const {
            return m_writeq.size();
        }

        void push_message_received_cb(
...
//        std::cout << "set " << key <<"val=" << val<<std::endl;
    ...
    private:
        // boost-arm::asio::ip::tcp::socket m_socket; // underlying socket
        std::shared_ptr<boost::asio::ip::tcp::socket> p_m_socket;
        ...
        std::vector<boost::function<void(message_ptr, connection_ptr)>>
                m_message_received_cbs;
        /// boost-arm::mutex m_message_received_cb_mutex; // protects the above
        Router *m_router;
    };
