 ...
 * @return
 */
int lntee::Command::load_enclave(const char *con_path,
                                 const char *enc) {
    oe_result_t result;
    // Create the enclave
...
//    std::cout <<"---> buf = "<< buf[0]<<" "<< <<std::endl;
    ...
    ecall_lntee_init_tee(Global::enclave,
                         ...
                         Global::addr,
                         ADDRESS_HEX_LEN);
//    std::cout << "Global::pubkey " <<Global::pubkey <<std::endl;
//    std::cout << "Global::addr " <<Global::addr <<std::endl;
    ...
    std::cout << "---> End loading contrat to enclave" << std::endl;
    return ret;
}
 ...
 * Load the contract
 */
void lntee::Command::load_contract(std::string con_path,
    ...
    DEBUG("");
    ecall_lntee_main(Global::enclave, str.c_str(),
                     (char *) &contractAddress,
                     sizeof(contractAddress));
 ...
 */
void lntee::Command::direct_send(std::string target,
                                 int amt) {
    DEBUG("DIRECT SEND");
...
//    for (int i = 0; i < 10000; i++) {
        ecall_lntee_direct_send(Global::enclave,
                                (const char *) pubkey,
                                amt,
                                (char *) tx,
                                16 + 64 + 32 + 32);
...
//    time_log("End the direct send");
    INFO();
    int res = 0;
    ecall_lntee_generate_contract_instance_id(Global::enclave,
                                              &res);
...
void lntee::Command::direct_recv(const char *tx_hex) {
    ...
    ecall_lntee_direct_recv(Global::enclave,
                            &ret,
                            tx);
 ...
 */
void lntee::Command::send_contract_tx(std::string target,
                                      const char *tx,
                                      int len) {
    DEBUG("");
    ...
    std::cout << "Function call "<<tx<<std::endl;
//    for (int i = 0; i < 10000; i++) {
        ecall_lntee_send(Global::enclave,
                         tx,
                         (char *) sign_tx,
                         512);
//    }
...
void lntee::Command::recv_contract_tx(const char *tx) {
    ...
    int ret = 0;
    ecall_lntee_recv_transaction(Global::enclave,
                                 &ret,
                                 tx);
