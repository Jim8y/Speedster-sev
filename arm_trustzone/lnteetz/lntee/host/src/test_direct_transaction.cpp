...
//#include <boost-arm/asio.hpp>
//#include <boost-arm/thread/thread.hpp>
//#include <boost-arm/algorithm/string.hpp>
...
void iorun(boost-arm::asio::io_service *ios)
{
    ios->run();
    cout << "io ended" << endl;
}
...
void command_parser(Router &r, std::string line)
{
    ...
    shm_command();
    vector<string> parts;
    boost-arm::split(parts, line, boost-arm::is_any_of(" "));
...
int main(int argc, const char *argv[])
{
    ...
    boost-arm::shared_ptr<boost-arm::asio::ip::tcp::acceptor> accp(
        new boost-arm::asio::ip::tcp::acceptor(ios, boost-arm::asio::ip::tcp::endpoint(
                                                    boost-arm::asio::ip::tcp::v4(),
                                                    port)));
