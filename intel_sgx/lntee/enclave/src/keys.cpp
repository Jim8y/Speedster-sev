...
int ecall_dispatcher::cipher_data(
    ...
    if (ret != 0) {
        std::cout << "mbedtls_aes_setkey_enc failed with " << ret << std::endl;
        goto exit;
    }
    ...
    ret = mbedtls_aes_crypt_cbc(
            ...
            output_data);
    if (ret != 0) {
        std::cout << "mbedtls_aes_crypt_cbc failed with " << ret << std::endl;
        goto exit;
    }
    exit:
    // free aes context
...
oe_result_t ecall_dispatcher::get_seal_key_by_policy(
    ...
    //    std::cout << "get_seal_key_by_policy: " << policy << std::endl;
...
int ecall_dispatcher::sign_sealed_data(
    ...
    if (ret)
        goto exit;
    ret = mbedtls_md_hmac_update(
            &ctx, (const unsigned char *) &(sealed_data->key_info_size), sizeof(size_t));
    if (ret)
        goto exit;
    ret = mbedtls_md_hmac_update(
            &ctx,
