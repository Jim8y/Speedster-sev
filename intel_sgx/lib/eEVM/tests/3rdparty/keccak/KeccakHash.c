...
HashReturn
Keccak_HashInitialize(Keccak_HashInstance *instance, unsigned int rate, unsigned int capacity, unsigned int hashbitlen,
...
HashReturn Keccak_HashUpdate(Keccak_HashInstance *instance, const BitSequence *data, BitLength databitlen) {
    if ((databitlen % 8) == 0)
        ...
        if (ret == SUCCESS) {
            ...
            unsigned short delimitedLastBytes = (unsigned short) ((unsigned short) lastByte |
